...
def run_optimization(PKI_model):
    ...
    for node in PKI_model.vertex_levels['l1']:
        if PKI_model.vertex_levels['l1'][node]:
            opt_graph.add_vertex(node)

    for key, value in PKI_model.edge_levels['l1'].items():
        for v in value:
    ...
    while True:
        ...
        if level_count == 1:
            for node in PKI_model.vertex_levels['l2']:
                if PKI_model.vertex_levels['l2'][node]:
                    opt_graph.add_vertex(node)
            for key, value in PKI_model.edge_levels['l2'].items():
                for v in value:
                    opt_graph.add_edge(key, v[0], v[1])

        if level_count == 2:
            for node in PKI_model.vertex_levels['l3']:
                if PKI_model.vertex_levels['l3'][node]:
                    opt_graph.add_vertex(node)
            for key, value in PKI_model.edge_levels['l3'].items():
                for v in value:
                    opt_graph.add_edge(key, v[0], v[1])
    ...
    path_a = find_path('s1', positive_flow_network, PKI_model.room_counts['s1'])
    path_b = find_path('s2', positive_flow_network, PKI_model.room_counts['s2'])
    path_c = find_path('s3', positive_flow_network, PKI_model.room_counts['s3'])
